"""
EXP54: Three Moves - CAGR Optimization Experiment
==================================================================================
Based on v8.4 COMPASS with THREE targeted mechanical changes to recover CAGR:

MOVE 1: ADOPT EXP45B WINNER (MOM_105d + WIDER TRAILING)
  - Momentum lookback: 90d -> 105d (exp45: +1.35% CAGR individually)
  - Trailing activation: 5% -> 8% (lets winners run further)
  - Trailing stop: 3% -> 5% (reduces whipsaw exits on winners)
  - exp45b combo result: 15.42% CAGR (+2.08% over v8.2 baseline)
  - Profit factor jumped to 1.491 (from 1.319)

MOVE 2: WIDEN DD SCALING TIER1 (-0.10 -> -0.15)
  - Stop throttling leverage on normal pullbacks (-10% DD happens yearly)
  - In 2023: v8.4 had 109 protection days at -10% tier -> +29.3%
             v8.2 had 0 protection days -> +63.4%
  - Expected: +0.5-1.0% CAGR from reduced idle time

MOVE 3: RE-ENABLE VOL-TARGETED LEVERAGE (MAX 1.5x)
  - v8.2 with leverage: 15.98% CAGR | v8.4 without: 12.19% CAGR
  - The 6% margin cost at avg 1.25x = 0.9%/year, but return uplift was 3-5%/year
  - Vol targeting already scales to 0.3x in crashes = built-in defense
  - Conservative cap at 1.5x (v8.2 went up to 2.0x)

MOVE 4: TIME-BASED RECOVERY FLOOR (fixes slow recovery trap)
  - Problem: smooth DD scaling keeps leverage suppressed for YEARS after crashes
  - 2002 crash: EXP54 stuck at 0.46x leverage for 3 years (2003-2005)
  - v8.2 binary stop recovered to full power in ~189 days via time-based reset
  - Solution: after 126 trading days in DD, guarantee DD scaling returns at least 1.0x
  - Crash velocity circuit breaker still overrides during active crashes
  - This combines smooth crash protection with time-based recovery speed

CONTROL: All other v8.4 logic preserved (sigmoid regime, adaptive stops,
sector limits, bull override, quality filter, renewal).

HYPOTHESIS: Combined moves should recover 3.0-4.5% CAGR over v8.4 baseline.
v8.4 baseline: 12.19% CAGR | Target: 15.0-16.5% CAGR
"""
...
# Signal
MOMENTUM_LOOKBACK = 105     # MOVE 1: 90->105d (exp45b winner, +1.35% CAGR individually)
...
TRAILING_ACTIVATION = 0.08  # MOVE 1: 5%->8% (exp45b winner, lets winners run)
TRAILING_STOP_PCT = 0.05    # MOVE 1: 3%->5% (exp45b winner, reduces whipsaw)
...
DD_SCALE_TIER1 = -0.10      # MOVE 2: -10%->-15% (stop throttling on normal pullbacks)
DD_SCALE_TIER2 = -0.20      # Medium drawdown (H3: widened from -15%)
DD_SCALE_TIER3 = -0.35      # Deep drawdown floor (H3: widened from -25%)
LEV_FULL       = 1.0        # MOVE 3 FIX: must match LEVERAGE_MAX so DD scaling doesn't cap vol targeting
...
# MOVE 4: Time-based recovery floor
RECOVERY_TIME_DAYS = 99999    # After 126 days in DD, guarantee at least LEV_RECOVERY_FLOOR
LEV_RECOVERY_FLOOR = 1.0    # Minimum leverage after recovery time (vol targeting still applies)

# Leverage & Vol targeting
TARGET_VOL = 0.15           # 15% anualizado
LEVERAGE_MAX = 1.0          # MOVE 3: 1.0->1.5x (re-enable vol-targeted leverage, conservative)
...
print("=" * 80)
print("EXP54: THREE MOVES - CAGR OPTIMIZATION EXPERIMENT")
print("Move 1: MOM 105d + Trail 8%/5% | Move 2: DD Tier1 -15% | Move 3: Lev 1.5x")
print("=" * 80)
...
print(f"Trailing: activation {TRAILING_ACTIVATION:.0%} / stop {TRAILING_STOP_PCT:.0%}")
print(f"Leverage: vol-targeted, max {LEVERAGE_MAX}x (margin cost {MARGIN_RATE:.0%})")
print(f"DD tiers: {DD_SCALE_TIER1}/{DD_SCALE_TIER2}/{DD_SCALE_TIER3}")
...
def _dd_leverage(drawdown: float) -> float:
    ...
    DD tiers (EXP54: tier1 widened to -15%, LEV_FULL = 1.5x):
      0%   to -15%: 1.5x (no DD reduction, vol targeting controls) -- MOVE 2+3
      -15% to -20%: 1.5x -> 0.60x (linear)
      -20% to -35%: 0.60x -> 0.30x (linear)
      < -35%:       0.30x (hard floor)
    """
    dd = drawdown
...
def compute_smooth_leverage(drawdown: float,
                             ...
                             current_idx: int,
                             crash_cooldown: int,
                             days_in_drawdown: int = 0) -> tuple:
    """
    Compute leverage via smooth drawdown scaling + crash velocity circuit breaker
    + MOVE 4: time-based recovery floor.

    After RECOVERY_TIME_DAYS in drawdown, DD scaling returns at least LEV_RECOVERY_FLOOR.
    Crash velocity circuit breaker still overrides during active crashes.
    ...
    # Normal DD scaling with MOVE 4: time-based recovery floor
    dd_lev = _dd_leverage(drawdown)

    if days_in_drawdown >= RECOVERY_TIME_DAYS and dd_lev < LEV_RECOVERY_FLOOR:
        dd_lev = LEV_RECOVERY_FLOOR

    return (dd_lev, updated_cooldown)
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    crash_cooldown = 0
    days_in_drawdown = 0  # MOVE 4: track consecutive days below peak
    ...
    for i, date in enumerate(all_dates):
        ...
        # MOVE 4: track consecutive days in drawdown
        if drawdown < -0.01:  # In meaningful drawdown (>1%)
            days_in_drawdown += 1
        else:
            days_in_drawdown = 0  # Reset when recovered to near-peak
        ...
        # --- Smooth drawdown leverage (MOVE 4: with time-based recovery floor) ---
...
if __name__ == "__main__":
    ...
    # 5. Print results
    print("\n" + "=" * 80)
    print("RESULTS - OMNICAPITAL v8.4 COMPASS")
    print("=" * 80)
    ...
    results['portfolio_values'].to_csv('backtests/exp54_signal_only_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp54_signal_only_trades.csv', index=False)

    output_file = 'results_v84_compass.pkl'
    ...
    print(f"Daily CSV: backtests/exp54_signal_only_daily.csv")
    print(f"Trades CSV: backtests/exp54_signal_only_trades.csv")

    # ==========================================================================
    # v8.4 IMPROVEMENT DIAGNOSTICS
    # ==========================================================================
    print("\n" + "=" * 80)
    print("v8.4 IMPROVEMENT DIAGNOSTICS")
    print("=" * 80)
    ...
    # ==========================================================================
    # EXP54: THREE MOVES vs BASELINES
    # ==========================================================================
    print("\n" + "=" * 80)
    print("EXP54: THREE MOVES vs ALL BASELINES")
    print("=" * 80)
    ...
    baselines = {
        'v8.2 BASE (lev ~2x)': {'CAGR': 0.1598, 'MaxDD': -0.2881, 'Sharpe': 0.770, 'Calmar': 0.557},
        'v8.3 (lev 1.0x)':     {'CAGR': 0.1154, 'MaxDD': -0.2962, 'Sharpe': 0.796, 'Calmar': 0.390},
        'v8.4 (lev 1.0x)':     {'CAGR': 0.1219, 'MaxDD': -0.2000, 'Sharpe': 0.800, 'Calmar': 0.400},
        'exp45b winner':        {'CAGR': 0.1542, 'MaxDD': -0.5908, 'Sharpe': 0.560, 'Calmar': 0.261},
    }

    for bname, bvals in baselines.items():
        print(f"\n--- EXP54 vs {bname} ---")
        print(f"{'Metric':<20} {'Baseline':>15} {'EXP54':>15} {'Delta':>15}")
        print("-" * 65)
        for key in bvals:
            b = bvals[key]
            v = exp54[key]
    ...
    print("=" * 80)
    gates = [
        ('CAGR >= 15.0%',           exp54['CAGR'] >= 0.15),
        ('CAGR > v8.4 (12.19%)',    exp54['CAGR'] > 0.1219),
        ('MaxDD > -40.0%',          exp54['MaxDD'] > -0.40),
        ('Sharpe >= 0.65',          exp54['Sharpe'] >= 0.65),
        ('Calmar >= 0.35',          exp54['Calmar'] >= 0.35),
    ]
    all_pass = True
    ...
    print(f"\nOverall: {'ALL GATES PASSED -- ADOPT' if all_pass else 'SOME GATES FAILED -- REVIEW'}")

    # Margin cost analysis
    print("\n" + "=" * 80)
    print("MARGIN COST ANALYSIS")
    print("=" * 80)
    df_pv = results['portfolio_values']
    if 'leverage' in df_pv.columns:
        avg_lev = df_pv['leverage'].mean()
        pct_above_1 = (df_pv['leverage'] > 1.0).mean() * 100
        avg_lev_when_above = df_pv.loc[df_pv['leverage'] > 1.0, 'leverage'].mean() if pct_above_1 > 0 else 0
        avg_borrowed_pct = max(0, (avg_lev - 1.0) / avg_lev) * 100 if avg_lev > 1.0 else 0
        est_annual_margin_cost = avg_borrowed_pct / 100 * MARGIN_RATE * 100
        print(f"  Average leverage:           {avg_lev:.3f}x")
        print(f"  Days with leverage > 1.0x:  {pct_above_1:.1f}%")
        print(f"  Avg leverage when > 1.0x:   {avg_lev_when_above:.3f}x")
        print(f"  Estimated margin cost:      {est_annual_margin_cost:.2f}% annual")
        print(f"  CAGR net of margin:         {metrics['cagr']:.2%} (already deducted in backtest)")

    print("\n" + "=" * 80)
    print("EXP54: THREE MOVES BACKTEST COMPLETE")
    print("=" * 80)
