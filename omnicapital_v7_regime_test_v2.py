"""
OmniCapital v7 - Regime-Based Leverage Test v2
...
def load_data():
    ...
    # Asegurar que todos los indices sean tz-naive
    if hasattr(vix_data.index, 'tz') and vix_data.index.tz:
        vix_data.index = vix_data.index.tz_localize(None)
    if hasattr(spy_data.index, 'tz') and spy_data.index.tz:
        spy_data.index = spy_data.index.tz_localize(None)
    if hasattr(prices_df.index, 'tz') and prices_df.index.tz:
    ...
    print(f"Datos cargados: {len(prices_df)} dias")
    return prices_df
