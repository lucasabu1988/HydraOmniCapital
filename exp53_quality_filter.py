"""
OmniCapital v8.3 FINAL - Quality Filter + Comparison (exp53)
==================================================================================
Nuevo algoritmo con signal real, regime filter, vol targeting, y position-level stops.

Basado en v8 COMPASS con parametros validados por exp45b (COMBO_MOM105_WTRAIL = 15.42% CAGR):
- Signal: Cross-sectional momentum (105d) + short-term reversal (5d skip)
...
- Exits: Hold time (5d) + position stop (-8%) + trailing stop (5% desde max, activa tras +8%)
- Smooth Drawdown Scaling: Piecewise-linear leverage reduction + crash velocity circuit breaker
- Sigmoid Regime: Gradual position reduction 5->4->3->2 instead of cliff 5->2
- Exit Renewal: Winners can extend hold if profit > 4%, score in top 85%, hard cap 10d
- Quality Filter: Exclude stocks with vol > 60% or single-day moves > 50% (for universe expansion)
"""
...
print("=" * 80)
print("OMNICAPITAL v8.3 FINAL - Quality Filter + Final Comparison")
print("Quality filter for universe expansion + v8.3 vs v8.2 comparison - exp53")
print("=" * 80)
...
print(f"Regime: Sigmoid score (trend 60% + vol 40%) | Vol target: {TARGET_VOL:.0%}")
print(f"Hold: {HOLD_DAYS}d | Pos stop: {POSITION_STOP_LOSS:.0%} | DD tiers: {DD_SCALE_TIER1}/{DD_SCALE_TIER2}/{DD_SCALE_TIER3}")
...
def regime_score_to_positions(regime_score: float,
                               ...
                               num_positions_risk_off: int = NUM_POSITIONS_RISK_OFF) -> int:
    """
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    for i, date in enumerate(all_dates):
        ...
        # Max positions from regime score (gradual)
...
if __name__ == "__main__":
    ...
    # 5. Print results
    print("\n" + "=" * 80)
    print("RESULTS - OMNICAPITAL v8.3 FINAL (exp53)")
    print("=" * 80)
    ...
    results['portfolio_values'].to_csv('backtests/exp53_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp53_trades.csv', index=False)

    output_file = 'results_exp53.pkl'
    ...
    print(f"Daily CSV: backtests/exp53_daily.csv")
    print(f"Trades CSV: backtests/exp53_trades.csv")

    # ==========================================================================
    # v8.3 vs v8.2 COMPARISON
    # ==========================================================================
    print("\n" + "=" * 80)
    print("COMPASS v8.3 vs v8.2 COMPARISON")
    print("=" * 80)

    # v8.2 baseline (from PROJECT_STATE.md)
    baseline = {
        'CAGR': 0.1390,
        'MaxDD': -0.6625,
        'Sharpe': 0.646,
    ...
    v83 = {
        ...
        'Stop Events': metrics.get('stop_events', 0),
    }

    print(f"\n{'Metric':<20} {'v8.2 Baseline':>15} {'v8.3 Result':>15} {'Delta':>15}")
    print("-" * 65)
    for key in baseline:
        b = baseline[key]
        v = v83[key]
        if key == 'Stop Events':
            print(f"{key:<20} {b:>15.0f} {v:>15.0f} {v - b:>+15.0f}")
        else:
    ...
    print("\n--- Go/No-Go Gates ---")
    gates = [
        ('CAGR >= 15.0%', v83['CAGR'] >= 0.15),
        ('MaxDD > -55.0%', v83['MaxDD'] > -0.55),
        ('Sharpe >= 0.72', v83['Sharpe'] >= 0.72),
        ('Stop Events <= 3', v83['Stop Events'] <= 3),
    ]
    all_pass = True
