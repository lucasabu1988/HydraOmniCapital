...
DD_SCALE_TIER1 = -0.15      # MOVE 2: -10%->-15% (stop throttling on normal pullbacks)
DD_SCALE_TIER2 = -0.20      # Medium drawdown (H3: widened from -15%)
DD_SCALE_TIER3 = -0.35      # Deep drawdown floor (H3: widened from -25%)
LEV_FULL       = 1.5        # MOVE 3 FIX: must match LEVERAGE_MAX so DD scaling doesn't cap vol targeting
...
RECOVERY_TIME_DAYS = 126    # After 126 days in DD, guarantee at least LEV_RECOVERY_FLOOR
...
LEVERAGE_MAX = 1.5          # MOVE 3: 1.0->1.5x (re-enable vol-targeted leverage, conservative)
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp54_three_moves_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp54_three_moves_trades.csv', index=False)
    ...
    print(f"Daily CSV: backtests/exp54_three_moves_daily.csv")
    print(f"Trades CSV: backtests/exp54_three_moves_trades.csv")
