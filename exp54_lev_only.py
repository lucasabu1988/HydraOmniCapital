...
# Signal
MOMENTUM_LOOKBACK = 90     # MOVE 1: 90->105d (exp45b winner, +1.35% CAGR individually)
...
TRAILING_ACTIVATION = 0.05  # MOVE 1: 5%->8% (exp45b winner, lets winners run)
TRAILING_STOP_PCT = 0.03    # MOVE 1: 3%->5% (exp45b winner, reduces whipsaw)
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp54_lev_only_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp54_lev_only_trades.csv', index=False)
    ...
    print(f"Daily CSV: backtests/exp54_lev_only_daily.csv")
    print(f"Trades CSV: backtests/exp54_lev_only_trades.csv")
