"""
OmniCapital v8.5 COMPASS CANDIDATE — Vol Compression Regime Flag (Idea 10)
==================================================================================
Based on v8.4 COMPASS with ONE targeted improvement:

IDEA 10: Vol Compression Regime Flag
  When VIX < 15 for >= 30 consecutive trading days, tighten crash brake:
    - 5d threshold: -6% -> -5%
    - 10d threshold: -10% -> -8%
  When VIX >= 15: reset to standard thresholds (no hysteresis).

  Rationale: Extended low-vol regimes (VIX < 15 for 30+ days) historically
  precede vol expansions (Ornstein-Uhlenbeck mean reversion). Tightening
  the crash brake during these periods catches the onset of crashes earlier.

  Pre-specified parameters (NOT optimized):
    VOL_COMPRESS_VIX_THRESHOLD = 15.0  (literature: Whaley 2000)
    VOL_COMPRESS_MIN_DAYS = 30         (minimum to distinguish regime from transient)
    VOL_COMPRESS_CRASH_5D = -0.05      (tightened from -0.06)
    VOL_COMPRESS_CRASH_10D = -0.08     (tightened from -0.10)

  Net new free parameters: 0

All v8.4 logic preserved unchanged.

Pre-registered success criteria (from decision doc 2026-03-01):
  - OOS CAGR improvement >= +40 bps vs baseline
  - Max drawdown improves or neutral (within +/- 50 bps)
  - Result must hold when 2008 excluded
  - Sharpe must not degrade

Pre-registered kill criteria:
  - OOS CAGR improvement < +15 bps
  - Max drawdown worsens by more than 100 bps
  - False alarm rate > 60% (flag fires but no crash within 90 days)
  - Improvement driven entirely by 2006-2007 episode
"""
...
# v8.5 Idea 10: Vol Compression Regime Flag
# Pre-specified values — NOT optimized. Source: Whaley (2000), OU mean reversion.
VOL_COMPRESS_VIX_THRESHOLD = 15.0   # VIX level that defines "compression"
VOL_COMPRESS_MIN_DAYS = 30          # Consecutive days below threshold to activate
VOL_COMPRESS_CRASH_5D = -0.05       # Tightened 5d crash brake (from -0.06)
VOL_COMPRESS_CRASH_10D = -0.08      # Tightened 10d crash brake (from -0.10)
...
print("=" * 80)
print("OMNICAPITAL v8.5 CANDIDATE — Vol Compression Regime Flag (Idea 10)")
print("v8.4 base + tightened crash brake when VIX < 15 for 30+ days")
print("=" * 80)
...
print(f"Vol compression: VIX < {VOL_COMPRESS_VIX_THRESHOLD} for {VOL_COMPRESS_MIN_DAYS}+ days -> crash 5d={VOL_COMPRESS_CRASH_5D:.0%}, 10d={VOL_COMPRESS_CRASH_10D:.0%}")
...
def download_vix() -> pd.DataFrame:
    """Download VIX data for vol compression flag"""
...
def compute_vol_compression_flag(vix_data: pd.DataFrame) -> pd.Series:
    """
    v8.5 Idea 10: Compute vol compression flag.
    Flag = 1 when VIX < 15.0 for >= 30 consecutive trading days.
    Flag = 0 when VIX >= 15.0 (immediate reset, no hysteresis).
    """
    vix_close = vix_data['Close']
    below_threshold = vix_close < VOL_COMPRESS_VIX_THRESHOLD

    # Count consecutive days below threshold
    flag = pd.Series(index=vix_close.index, dtype=bool, data=False)
    consecutive = 0

    for i in range(len(vix_close)):
        if below_threshold.iloc[i]:
            consecutive += 1
            if consecutive >= VOL_COMPRESS_MIN_DAYS:
                flag.iloc[i] = True
        else:
            consecutive = 0
            # No hysteresis — immediate reset
...
def compute_smooth_leverage(drawdown: float,
                             ...
                             current_idx: int,
                             crash_cooldown: int,
                             vol_compress_active: bool = False) -> tuple:
    """
    v8.5 Idea 10: Compute leverage via smooth drawdown scaling + crash velocity
    circuit breaker. When vol_compress_active=True, use tightened thresholds.

    Standard thresholds:  5d = -6%, 10d = -10%
    Compressed thresholds: 5d = -5%, 10d = -8%
    """
    def _val(entry):
    ...
    # Select thresholds based on vol compression state
    crash_5d = VOL_COMPRESS_CRASH_5D if vol_compress_active else CRASH_VEL_5D
    crash_10d = VOL_COMPRESS_CRASH_10D if vol_compress_active else CRASH_VEL_10D
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
                 ...
                 vol_compress_flag: Optional[pd.Series] = None) -> Dict:
    """Run COMPASS v8.5 Idea 10 backtest (vol compression crash brake)"""
    ...
    # v8.5 Idea 10: Vol compression tracking
    vol_compress_days = 0          # total days flag was active
    vol_compress_triggers = []     # log of crash brakes fired during compression
    ...
    for i, date in enumerate(all_dates):
        ...
        # --- v8.5 Idea 10: Check vol compression flag ---
        vc_active = False
        if vol_compress_flag is not None and date in vol_compress_flag.index:
            vc_active = bool(vol_compress_flag.loc[date])
        if vc_active:
            vol_compress_days += 1

        # --- Smooth drawdown leverage (v8.5: with vol compression) ---
        ...
        dd_leverage_val, crash_cooldown = compute_smooth_leverage(
            ...
            vol_compress_active=vc_active
        )

        # Log crash brakes that fired during vol compression
        if prev_cooldown == 0 and crash_cooldown > 0 and vc_active:
            vol_compress_triggers.append({
                'date': date,
                'drawdown': drawdown,
                'crash_type': 'vol_compressed',
            })
        elif prev_cooldown == 0 and crash_cooldown > 0 and not vc_active:
            vol_compress_triggers.append({
                'date': date,
                'drawdown': drawdown,
                'crash_type': 'standard',
    ...
    return {
        ...
        'vol_compress_triggers': vol_compress_triggers,
    }
...
if __name__ == "__main__":
    ...
    # 1b. Download VIX and compute vol compression flag
    vix_data = download_vix()
    print(f"VIX data: {len(vix_data)} trading days")
    vc_flag = compute_vol_compression_flag(vix_data)
    vc_active_days = int(vc_flag.sum())
    print(f"Vol compression active: {vc_active_days}/{len(vc_flag)} days ({vc_active_days/len(vc_flag)*100:.1f}%)")
    ...
    # 5. Print results
    print("\n" + "=" * 80)
    print("RESULTS - OMNICAPITAL v8.5 COMPASS CANDIDATE (Idea 10)")
    print("=" * 80)
    ...
    results['portfolio_values'].to_csv('backtests/v85_idea10_vol_compression_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/v85_idea10_vol_compression_trades.csv', index=False)

    output_file = 'results_v85_idea10_vol_compression.pkl'
    with open(output_file, 'wb') as f:  # pickle: same format as v8.3
        pickle.dump({
            'params': {
                ...
                'vol_compress_vix': VOL_COMPRESS_VIX_THRESHOLD,
                'vol_compress_days': VOL_COMPRESS_MIN_DAYS,
                'vol_compress_crash_5d': VOL_COMPRESS_CRASH_5D,
                'vol_compress_crash_10d': VOL_COMPRESS_CRASH_10D,
                'version': 'v8.5_candidate_idea10_vol_compression',
            },
            'metrics': metrics,
    ...
    print(f"Daily CSV: backtests/v85_idea10_vol_compression_daily.csv")
    print(f"Trades CSV: backtests/v85_idea10_vol_compression_trades.csv")

    # ==========================================================================
    # v8.5 IDEA 10 — VOL COMPRESSION DIAGNOSTICS
    # ==========================================================================
    print("\n" + "=" * 80)
    print("v8.5 IDEA 10 — VOL COMPRESSION DIAGNOSTICS")
    print("=" * 80)

    df_pv = results['portfolio_values']
    total_days = len(df_pv)

    # Vol compression stats
    vc_days = results.get('vol_compress_days', 0)
    vc_triggers = results.get('vol_compress_triggers', [])
    print(f"\n--- Vol Compression Flag Stats ---")
    print(f"Compression active days:    {vc_days:>6} / {total_days} ({vc_days/total_days*100:.1f}%)")
    print(f"VIX threshold:              {VOL_COMPRESS_VIX_THRESHOLD}")
    print(f"Min consecutive days:       {VOL_COMPRESS_MIN_DAYS}")
    print(f"Tightened 5d brake:         {VOL_COMPRESS_CRASH_5D:.0%} (from {CRASH_VEL_5D:.0%})")
    print(f"Tightened 10d brake:        {VOL_COMPRESS_CRASH_10D:.0%} (from {CRASH_VEL_10D:.0%})")

    # Crash brake events breakdown
    compressed_triggers = [t for t in vc_triggers if t['crash_type'] == 'vol_compressed']
    standard_triggers = [t for t in vc_triggers if t['crash_type'] == 'standard']
    print(f"\n--- Crash Brake Events ---")
    print(f"Total crash brakes:         {len(vc_triggers)}")
    print(f"  During vol compression:   {len(compressed_triggers)} (with tightened thresholds)")
    print(f"  Standard:                 {len(standard_triggers)} (normal thresholds)")

    if compressed_triggers:
        print(f"\nCrash brakes fired during vol compression:")
        for t in compressed_triggers:
            d = t['date'].strftime('%Y-%m-%d') if hasattr(t['date'], 'strftime') else str(t['date'])
            print(f"  {d}  DD: {t['drawdown']:.1%}")

    if standard_triggers:
        print(f"\nCrash brakes fired with standard thresholds:")
        for t in standard_triggers:
            d = t['date'].strftime('%Y-%m-%d') if hasattr(t['date'], 'strftime') else str(t['date'])
            print(f"  {d}  DD: {t['drawdown']:.1%}")

    # ==========================================================================
    # v8.5 vs v8.4 — PRE-REGISTERED EVALUATION
    # ==========================================================================
    print("\n" + "=" * 80)
    print("v8.5 (Idea 10) vs v8.4 — PRE-REGISTERED EVALUATION")
    print("=" * 80)
    ...
    # Success criteria
    cagr_success = cagr_delta >= 0.0040   # +40 bps
    maxdd_ok = abs(maxdd_delta) <= 0.0050  # within +/- 50 bps
    sharpe_ok = v85['Sharpe'] >= baseline_v84['Sharpe'] - 0.02  # not degraded

    print(f"  CAGR delta >= +40 bps:    {cagr_delta*10000:+.0f} bps — {'PASS' if cagr_success else 'FAIL'}")
    print(f"  MaxDD within +/- 50 bps:  {maxdd_delta*10000:+.0f} bps — {'PASS' if maxdd_ok else 'FAIL'}")
    print(f"  Sharpe not degraded:      {v85['Sharpe']:.3f} vs {baseline_v84['Sharpe']:.3f} — {'PASS' if sharpe_ok else 'FAIL'}")

    # Kill criteria
    cagr_kill = cagr_delta < 0.0015       # < +15 bps
    maxdd_kill = maxdd_delta < -0.0100    # worsens by >100 bps

    # False alarm rate
    if compressed_triggers:
        # Check each compressed trigger: did a >-10% drawdown follow within 90 days?
        false_alarms = 0
        for t in compressed_triggers:
            trig_idx = df_pv.index[df_pv['date'] == t['date']].tolist()
            if not trig_idx:
                continue
            idx = trig_idx[0]
            future = df_pv.iloc[idx:idx+90] if idx+90 < len(df_pv) else df_pv.iloc[idx:]
            if future['drawdown'].min() > -0.10:
                false_alarms += 1
        false_alarm_rate = false_alarms / len(compressed_triggers) if compressed_triggers else 0
        print(f"  False alarm rate:         {false_alarms}/{len(compressed_triggers)} ({false_alarm_rate:.0%}) — {'FAIL (>60%)' if false_alarm_rate > 0.60 else 'PASS'}")
        false_alarm_kill = false_alarm_rate > 0.60
    else:
        print(f"  False alarm rate:         N/A (no compressed triggers)")
        false_alarm_kill = False

    if cagr_kill:
        print(f"\n  KILL: CAGR improvement < +15 bps ({cagr_delta*10000:+.0f} bps)")
    if maxdd_kill:
        print(f"\n  KILL: MaxDD worsened by > 100 bps ({maxdd_delta*10000:+.0f} bps)")

    # Overall verdict
    all_success = cagr_success and maxdd_ok and sharpe_ok
    any_kill = cagr_kill or maxdd_kill or false_alarm_kill
    ...
    print("\n" + "=" * 80)
    print("COMPASS v8.5 IDEA 10 BACKTEST COMPLETE")
    print("=" * 80)
