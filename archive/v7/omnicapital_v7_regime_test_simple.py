"""
OmniCapital v7 - Regime-Based Leverage Test (Simplified)
Test de leverage adaptativo basado en VIX - Version simplificada
"""
...
# Universo reducido para test rapido
UNIVERSE = ['AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META', 'TSLA', 'NVDA', 'JPM', 'V', 'JNJ']


def load_data_simple():
    """Carga datos de CSV individuales"""
    ...
    prices_df['VIX'] = vix['Close']
    prices_df['VIX'] = prices_df['VIX'].ffill()
    
    # Descargar SPY para benchmark
    spy = yf.download('SPY', start='1999-01-01', end='2026-02-10', progress=False)
    prices_df['SPY'] = spy['Close']
    
    # Filtrar fechas - convertir a tz-naive si es necesario
    ...
    prices_df = prices_df[prices_df.index >= start_ts]
    prices_df = prices_df[prices_df.index <= end_ts]
...
def run_backtest(prices_df, use_regime=True, verbose=False):
    """Ejecuta backtest"""
    ...
    for i, (date, row) in enumerate(prices_df.iterrows()):
        ...
        if slots > 0 and not in_protection:
            available = [s for s in stock_cols 
                        if not pd.isna(row.get(s)) and s not in positions]
...
def main():
    ...
    m_v6 = analyze(df_v6, events_v6, stats_v6, "v6")
    ...
    m_v7 = analyze(df_v7, events_v7, stats_v7, "v7")
    ...
    if improvement > 0.005:
        ...
        print("  RECOMENDACION: Mantener v6 (simplicidad)")
    else:
        print(f"✗ v7 EMPEORA v6 en {improvement:.2%} CAGR")
        print("  RECOMENDACION: Descartar")
