...
function fetchExpAnalysis() {
    ...
    html += '<div class="exp-stat-box"><div class="exp-stat-label">CAGR BASELINE</div><div class="exp-stat-val" style="color:var(--green);">13.90%</div><div class="exp-stat-note">Survivorship-corrected | ~11.40% net (after 2.5% costs)</div></div>';
...
/* ============ HERO ALPHA CHART (Carousel Slide 4) ============ */
var _heroAlphaChart = null;
async function renderHeroAlphaChart() {
    var canvas = document.getElementById('heroAlphaChart');
    if (!canvas) return;
    try {
        var res = await fetch('/api/annual-returns');
        ...
        json.data.forEach(function(d) {
            if (d.spy != null) {
                years.push(d.year);
                alphas.push(Math.round((d.compass - d.spy) * 10) / 10);
            }
        });
        var isDark = document.documentElement.getAttribute('data-theme') !== 'light';
        var colors = alphas.map(function(a) {
            return a >= 0
                ? (isDark ? 'rgba(0, 230, 118, 0.7)' : 'rgba(0, 200, 83, 0.7)')
                : (isDark ? 'rgba(255, 82, 82, 0.55)' : 'rgba(229, 57, 53, 0.55)');
        });
        if (_heroAlphaChart) _heroAlphaChart.destroy();
        _heroAlphaChart = new Chart(canvas.getContext('2d'), {
            type: 'bar',
            data: {
                labels: years,
                datasets: [{
            ...
            options: {
                ...
                plugins: {
                    legend: { display: false },
                    tooltip: {
                        backgroundColor: isDark ? 'rgba(14,14,30,0.95)' : 'rgba(255,255,255,0.95)',
...
document.addEventListener('DOMContentLoaded', function() { renderHeroAlphaChart(); });
