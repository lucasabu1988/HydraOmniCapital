"""
EXP58: Tail Hedge Overlay + Relaxed DD Scaling
==================================================================================
Based on v8.4 COMPASS with TWO changes:

1. SYNTHETIC PUT SPREAD OVERLAY: Monthly SPY put spread (5% OTM / 20% OTM)
   - Cost: estimated via Black-Scholes using realized vol as proxy for IV
   - Payoff: linear between -5% and -20% SPY monthly return
   - Deducted from cash each month as insurance cost

2. RELAXED DD SCALING: Because the overlay provides explicit crash protection,
   widen the DD tiers to reduce the recovery trap:
   - T1: -10% -> -15% (start reducing leverage later)
   - T2: -20% -> -30%
   - T3: -35% -> -45%

This addresses the core bottleneck: DD scaling consumes 25-28% of days at
reduced leverage, killing compounding during recoveries.
"""
...
REGRESSION_WINDOW = 252     # Rolling OLS window for beta estimation (1 year)
...
# EXP58: Only relax T1 (most impactful tier — triggers most often)
DD_SCALE_TIER1 = -0.20      # Aggressively relaxed (don't start reducing until -20%)
DD_SCALE_TIER2 = -0.20      # Keep original
DD_SCALE_TIER3 = -0.35      # Keep original
LEV_FULL       = 1.0        # Full leverage (no reduction)
LEV_MID        = 0.60       # Medium drawdown leverage
LEV_FLOOR      = 0.30       # Hard floor

# EXP58: Put spread overlay parameters (cheap, conditional)
PUT_SPREAD_LONG_OTM = 0.10   # Long put: 10% OTM (cheaper)
PUT_SPREAD_SHORT_OTM = 0.25  # Short put: 25% OTM
PUT_SPREAD_REBALANCE_DAYS = 21  # Monthly rebalance (trading days)
PUT_SPREAD_NOTIONAL_PCT = 0.50  # Hedge 50% of portfolio (cheaper)
PUT_SPREAD_VOL_THRESHOLD = 0.18  # Only buy when realized vol > 18% (conditional)
...
print("=" * 80)
print("OMNICAPITAL v8.4 COMPASS - 3 Algorithmic Improvements")
print("Bull Override | Adaptive Stops | Sector Limits")
print("=" * 80)
print(f"\nBroad pool: {len(BROAD_POOL)} stocks | Top-{TOP_N} annual rotation")
print(f"Signal: Momentum {MOMENTUM_LOOKBACK}d (skip {MOMENTUM_SKIP}d) + Inverse Vol sizing + PUT SPREAD OVERLAY")
...
def compute_momentum_scores(price_data: Dict[str, pd.DataFrame],
    ...
    """
    BASELINE: Original v8.4 risk-adjusted momentum (raw return / vol).
    """
    scores = {}
...
# ============================================================================
# EXP58: PUT SPREAD OVERLAY
# ============================================================================

from scipy.stats import norm

def black_scholes_put(S, K, T, r, sigma):
    """Black-Scholes put price. S=spot, K=strike, T=years, r=rate, sigma=vol."""
    if T <= 0 or sigma <= 0:
        return max(K - S, 0)
...
def compute_put_spread_cost(spy_price, realized_vol, risk_free_rate=0.04):
    """
    Compute monthly put spread cost as % of notional.
    Long put at 5% OTM, short put at 20% OTM, 1-month expiry.
    Uses realized vol * 1.2 as IV proxy (vol risk premium).
    """
    iv = realized_vol * 1.2  # IV typically 20% above realized
    T = PUT_SPREAD_REBALANCE_DAYS / 252  # ~1 month

    long_strike = spy_price * (1 - PUT_SPREAD_LONG_OTM)
    short_strike = spy_price * (1 - PUT_SPREAD_SHORT_OTM)

    long_put = black_scholes_put(spy_price, long_strike, T, risk_free_rate, iv)
    short_put = black_scholes_put(spy_price, short_strike, T, risk_free_rate, iv)

    spread_cost = long_put - short_put
    return spread_cost / spy_price  # as % of spot


def compute_put_spread_payoff(spy_return_monthly):
    """
    Compute put spread payoff for a given monthly SPY return.
    Pays linearly between -5% and -20%, maxes out at -20%.
    Returns payoff as % of notional.
    """
    if spy_return_monthly >= -PUT_SPREAD_LONG_OTM:
        return 0.0  # OTM, no payoff
    elif spy_return_monthly <= -PUT_SPREAD_SHORT_OTM:
        # Max payoff = spread width
        return PUT_SPREAD_SHORT_OTM - PUT_SPREAD_LONG_OTM  # 15%
    else:
        # Linear payoff between strikes
        return -spy_return_monthly - PUT_SPREAD_LONG_OTM
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    # EXP58: Overlay state
    overlay_last_rebalance_idx = 0
    overlay_entry_spy_price = None
    overlay_total_cost = 0.0
    overlay_total_payoff = 0.0
    ...
    for i, date in enumerate(all_dates):
        ...
        # --- EXP58: Put spread overlay (monthly) ---
        if date in spy_data.index:
            spy_price_now = spy_data.loc[date, 'Close']
            if i - overlay_last_rebalance_idx >= PUT_SPREAD_REBALANCE_DAYS and i > 0:
                # DISABLED: no overlay payoff (testing DD scaling relaxation alone)
                pass

                # Buy new put spread for next period (conditional on vol)
                spy_idx = spy_data.index.get_loc(date)
                if spy_idx >= VOL_LOOKBACK + 1:
                    spy_rets = spy_data['Close'].iloc[spy_idx - VOL_LOOKBACK:spy_idx + 1].pct_change().dropna()
                    realized_vol = float(spy_rets.std() * (252 ** 0.5))
                else:
                    realized_vol = 0.20

                # DISABLED: testing relaxed DD scaling alone (no hedge cost)
                pass

                overlay_entry_spy_price = spy_price_now
                overlay_last_rebalance_idx = i
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp58_tail_hedge_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp58_tail_hedge_trades.csv', index=False)

    output_file = 'results_exp58_tail_hedge.pkl'
    ...
    print(f"Daily CSV: backtests/exp58_tail_hedge_daily.csv")
    print(f"Trades CSV: backtests/exp58_tail_hedge_trades.csv")
    ...
    # ==========================================================================
    # v8.4 vs v8.3 COMPARISON
    # ==========================================================================
    print("\n" + "=" * 80)
    print("COMPASS v8.4 vs v8.3 COMPARISON")
    print("=" * 80)

    # v8.3 baseline (from actual v83_compass_daily.csv)
    baseline_v83 = {
        'CAGR': 0.1157,
        'MaxDD': -0.2962,
        'Sharpe': 0.7955,
    ...
    print(f"\n{'Metric':<20} {'v8.3 Baseline':>15} {'v8.4 Result':>15} {'Delta':>15}")
    print("-" * 65)
    ...
    # v8.2 comparison (historical)
    print("\n" + "-" * 65)
    print(f"{'Metric':<20} {'v8.2 Baseline':>15} {'v8.4 Result':>15} {'Delta':>15}")
    print("-" * 65)
    baseline_v82 = {
        'CAGR': 0.1390,
        'MaxDD': -0.6625,
    ...
    print("\n--- Go/No-Go Gates ---")
    gates = [
        ('CAGR >= 15.0%', v84['CAGR'] >= 0.15),
        ('MaxDD > -55.0%', v84['MaxDD'] > -0.55),
        ('Sharpe >= 0.72', v84['Sharpe'] >= 0.72),
    ]
    all_pass = True
    ...
    print("\n" + "=" * 80)
    print("COMPASS v8.4 BACKTEST COMPLETE")
    print("=" * 80)
