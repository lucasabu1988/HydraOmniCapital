...
FILLED_STATUSES = ("filled", "confirmed", "confirmed_unplanned")
...
def lots_from_ledger(state: dict, *, statuses=("filled",)) -> dict:
    """(sleeve, tranche, ticker) -> {qty, cost_total, realised, fees}.

    `statuses` = which fill statuses count (the dashboard historically counted "filled" only;
    the attribution passes FILLED_STATUSES so confirmed fills count too)."""
    lots: dict = {}
    ...
    for fill in (state or {}).get("ledger") or []:
        if fill.get("status") not in statuses:
            continue
        side = fill.get("side")
