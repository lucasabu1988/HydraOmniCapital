...
| Disk loss / corrupt `state/portfolio_v9.json` | 1. `python verify_state.py --verify-generation <HYDRA_BACKUP_DIR>/state_v9/<YYYYMMDD>` (no run_id = use `LATEST`). It must report 0 findings, profile `daily_v9`, one `run_id` across every role. 2. `python verify_state.py --restore-into D:/tmp/drill --from-generation <that dir>` — an EMPTY directory, never `state/`; the tool refuses the live tree before creating anything. 3. Compare the restored book against the last sheet, then copy it in by hand. A generation that reports `GEN_NO_MANIFEST` predates TASK-395 and was never verifiable. |
