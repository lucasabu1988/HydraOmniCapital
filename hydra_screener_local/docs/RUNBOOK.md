...
| Disk loss / corrupt `state/portfolio_v9.json` | Restore from `HYDRA_BACKUP_DIR/state_v9/<date>/` (or `state/backup/`) — **check the generation first**: on 2026-09-13 three September generations (`20260904`, `20260905`, `20260908`) turned out to be pytest fixtures, and each now carries a `DO_NOT_RESTORE.txt`. The verified one is `20260904_LIVE_VERIFIED`, whose `backup_manifest.json` names the real `state/` as its source and whose five files were confirmed byte-for-byte against OneDrive on 2026-09-13. `LIVE_VERIFIED` means integrity of the copy of the observed local state and nothing more — not recovery from a second machine, not journal completeness, not broker fills. TASK-360 `verify_state.py` is the drill once it lands; until then diff the JSON against the last off-disk copy and the last sheet. |
