...
def test_v9_prints_the_name_and_still_hards_never_saves_last_ok(tmp_path, capsys, monkeypatch):
    ...
    prev = {"at": "2026-09-10T16:55:00Z", "universe": "all",
            "groups": {"etf": {"print_share": 1.0, "last_bar": "2026-09-10"}}}
    monkeypatch.setattr(V, "load_last_ok_print_quality", lambda universe=None, **k: prev)
    saved = []
...
def test_v9_first_run_still_names_a_low_share_without_a_prior_ok(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(V, "load_last_ok_print_quality", lambda universe=None, **k: None)
    saved = []
    monkeypatch.setattr(V, "save_last_ok_print_quality", lambda *a, **k: saved.append(1))
    idx = pd.DatetimeIndex(["2026-09-09", "2026-09-10"])
...
def test_a_good_run_still_records_itself(monkeypatch):
    ...
    monkeypatch.setattr(V, "save_last_ok_print_quality",
                        lambda groups, universe=None, **k: seen.update(groups=groups, universe=universe))
...
def test_run_hands_the_sidecar_its_destination_without_a_monkeypatch(tmp_path, capsys):
    """TASK-426: run() resolves runs_dir once and passes it. No module patch.

    The sidecar lives in tmp_path. If run() still called load() with no runs_dir, the
    conftest fence would send it to an empty temp runs/ and this would print the first-run
    line instead of 'vs last ok'.
    """
    import portfolio_v9 as V
    from config import V9
    from test_portfolio_v9_cli import FakeEngine, _rank

    prev_groups = {"etf": {"print_share": 1.0, "last_bar": "2026-09-10"}}
    save_last_ok_print_quality(prev_groups, universe="all", runs_dir=tmp_path,
                               at="2026-09-10T16:55:00Z")
    idx = pd.DatetimeIndex(["2026-09-09", "2026-09-10"])
    ...
    with pytest.raises(SystemExit):
        V.run(tmp_path, capital=100000.0, fetch_fn=market, rank_fn=_rank,
              engine=FakeEngine(), silent=False, runs_dir=tmp_path)
    out = capsys.readouterr().out
    assert "provider refresh degraded" in out
    assert "vs last ok" in out
    assert "no prior successful run" not in out


def test_save_writes_the_runs_dir_it_was_handed(tmp_path):
    """TASK-426: the writer uses the destination run() resolved, not a nested default."""
    import portfolio_v9 as V

    assert V._save_print_quality({"etf": {"print_share": 1.0}}, "all",
                                 silent=True, runs_dir=tmp_path) is True
    assert (tmp_path / "last_ok_print_quality.json").exists()
