"""Point-in-time universe and sector snapshots (TASK-362, hardened in audit phase 6, strictness audited in ASTRA-05).
...
class PitMissing(RuntimeError):
    """A point-in-time input an audited run needs is not on disk (phase 6.6 / audit ASTRA-05):
    the caller declared it requires the snapshot, and nothing is fabricated in its place."""
...
def require_sectors_at(date=None, *, pit_dir=None) -> tuple[dict, str, dict]:
    ...
    # identity = phase-6 content hash (sha256 / recorded_sha256 / verified) + the ASTRA-05 fields
    # (count / same_as) the lab records; shared keys agree.
    return sec, pick, {**snapshot_identity("sectors", pick, pit_dir=pit_dir),
                       **sectors_identity(pick, pit_dir=pit_dir)}
