...
def attribution(state: dict, marks: dict | None = None) -> dict:
    ...
    sleeves_out = {}
    # The accumulator is numeric; the two report-only keys (`change_pct`, which is None on an
    # empty book, and the boolean `transfers_net_zero`) are added to a copy at the end.
    book: dict[str, float] = dict(initial=0.0, value=0.0, change=0.0, selection=0.0, etf=0.0,
                                  trading_other=0.0, fees=0.0, interest=0.0, dividends=0.0,
                                  transfers=0.0, residual=0.0)
    ...
    report: dict[str, object] = dict(book)
    report["transfers_net_zero"] = abs(book["transfers"]) <= 1e-6
    report["change_pct"] = (book["change"] / book["initial"]) if book["initial"] else None
    return dict(book=report, sleeves=sleeves_out, positions=positions(state, marks),
