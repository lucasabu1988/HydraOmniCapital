...
- Baseline measured on `main` 2026-09-08 (`--strict-console`): **59 files pass, 0 skip, 126 s**, ruff
  clean. That is the **tracked** count: at the 2026-09-06 tip (`f017966`) the discovery globs matched
  47 + `validate_pine_contract.py` = 48, at this commit 58 + 1 = 59, so **eleven** discovered test files
  ...
  `test_path_momentum.py` from H-009. `run_all_tests.py --list` also picks up any **untracked**
