...
- Baseline measured on `main` 2026-09-08 (`--strict-console`): **60 files pass, 0 skip, 136 s**, ruff
  clean. That is the **tracked** count: at the 2026-09-06 tip (`f017966`) the discovery globs matched
  47 + `validate_pine_contract.py` = 48, at this commit 59 + 1 = 60, so **twelve** discovered test files
  ...
  `test_path_momentum.py` / `test_residual_momentum.py` from H-009 and H-010. `run_all_tests.py --list` also picks up any **untracked**
