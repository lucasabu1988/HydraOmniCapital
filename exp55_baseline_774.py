...
CHANGE: Instead of ranking by raw return / vol, we:
  1. Regress each stock's returns against SPY (rolling 252d OLS)
  2. Compute momentum as sum of residuals over lookback window
  3. Risk-adjust by dividing by residual volatility

This strips out market beta from the momentum signal, ranking stocks by
their idiosyncratic momentum only (Blitz et al. 2011).
...
print(f"Signal: RESIDUAL Momentum {MOMENTUM_LOOKBACK}d (skip {MOMENTUM_SKIP}d, reg {REGRESSION_WINDOW}d) + Inverse Vol sizing")
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp55_baseline_774_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp55_baseline_774_trades.csv', index=False)

    output_file = 'results_exp55_baseline_774.pkl'
    ...
    print(f"Daily CSV: backtests/exp55_baseline_774_daily.csv")
    print(f"Trades CSV: backtests/exp55_baseline_774_trades.csv")
