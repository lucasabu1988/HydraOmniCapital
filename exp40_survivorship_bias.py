"""
Experiment 40: Survivorship Bias Quantification for COMPASS v8.2
=================================================================

This script quantifies the survivorship bias in the COMPASS v8.2 backtest by:
1. Downloading historical S&P 500 constituents (point-in-time membership)
2. Obtaining price data for delisted/bankrupt stocks from multiple sources
3. Reconstructing the true historical universe (including stocks that failed)
4. Running the EXACT COMPASS v8.2 backtest with the corrected universe
5. Comparing results to quantify the survivorship bias in CAGR terms

Data Sources:
- Historical constituents: GitHub fja05680/sp500
- Delisted stock prices: Stooq (primary), yfinance (fallback), Alpha Vantage (fallback)
- Current stock prices: yfinance (as in original)
- SPY data: yfinance (unchanged)
- Cash yield: FRED Moody's Aaa (unchanged)

Output:
- Console: Detailed comparison report
- backtests/exp40_survivorship_daily.csv: Daily equity curve
- backtests/exp40_survivorship_trades.csv: All trades
- backtests/exp40_survivorship_analysis.csv: Impact of delisted stocks
- backtests/exp40_comparison.txt: Side-by-side metrics
"""
...
print("=" * 80)
print("EXPERIMENT 40: SURVIVORSHIP BIAS QUANTIFICATION")
print("=" * 80)
print(f"\nObjective: Quantify survivorship bias in COMPASS v8.2 backtest")
print(f"Method: Compare original (current S&P 500) vs corrected (historical point-in-time)")
...
def filter_anomalous_stocks(data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    ...
    - Extreme volatility (> 300% annualized)
    """
    ...
    for symbol, df in data.items():
        try:
            ...
            if max_single_day > 5.0:  # 500%+ single day
                is_valid = False
                reason = f"Extreme gain: {max_single_day*100:.0f}%"
            elif volatility > 3.0:  # 300%+ annualized vol
                is_valid = False
...
def compare_and_quantify_bias(results_original: Dict, results_corrected: Dict):
    ...
    with open('backtests/exp40_comparison.txt', 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("EXPERIMENT 40: SURVIVORSHIP BIAS QUANTIFICATION\n")
    ...
    # Save equity curves
    original_equity.to_csv('backtests/exp40_original_daily.csv', index=False)
    corrected_equity.to_csv('backtests/exp40_corrected_daily.csv', index=False)

    # Save trade logs
    pd.DataFrame(results_original['trades']).to_csv('backtests/exp40_original_trades.csv', index=False)
    pd.DataFrame(results_corrected['trades']).to_csv('backtests/exp40_corrected_trades.csv', index=False)

    print("\n[SAVED] Results saved to backtests/ directory:")
    print("  - exp40_comparison.txt (detailed comparison)")
    print("  - exp40_original_daily.csv (original equity curve)")
    print("  - exp40_corrected_daily.csv (corrected equity curve)")
    print("  - exp40_original_trades.csv (original trades)")
    print("  - exp40_corrected_trades.csv (corrected trades)")
...
def main():
    ...
    print("\nNext steps:")
    print("1. Review backtests/exp40_comparison.txt for detailed metrics")
    print("2. Check backtests/exp40_survivorship_analysis.csv for delisted stock impact")
    print("3. Review backtests/exp40_survivorship_daily.csv for equity curve")
