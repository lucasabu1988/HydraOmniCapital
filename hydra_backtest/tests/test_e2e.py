"""End-to-end tests for hydra_backtest. SLOW, marked @pytest.mark.slow.
...
def test_e2e_full_backtest_runs_to_completion(tmp_path):
    """Full backtest over the available PIT cache range runs without errors."""
    out = tmp_path / 'run1'
    ...
    # Summary table should mention all 5 waterfall tiers
...
def test_e2e_determinism(tmp_path):
    ...
    Uses a shorter 3-year window to keep test runtime reasonable.
    """
    out1 = tmp_path / 'run1'
    out2 = tmp_path / 'run2'
