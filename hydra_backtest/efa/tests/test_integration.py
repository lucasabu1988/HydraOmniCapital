...
def _make_synth_efa(n_days: int = 300) -> pd.DataFrame:
    """Trending-up synthetic EFA series with enough history for SMA200."""
    np.random.seed(666)
...
def test_cli_runs_end_to_end(tmp_path):
    ...
    daily_csv = out_dir / 'efa_v2_daily.csv'
    trades_csv = out_dir / 'efa_v2_trades.csv'
    waterfall_json = out_dir / 'efa_v2_waterfall.json'
