"""
EXP57: Dynamic Hold Time (Vol-Scaled)
==================================================================================
Based on v8.4 COMPASS with ONE change: hold time varies by stock volatility.

CHANGE: Instead of fixed 5-day hold for all stocks:
  - Low vol stocks: hold longer (up to 10d) to ride stable trends
  - High vol stocks: hold shorter (down to 3d) to avoid mean reversion
  - Formula: hold_days = round(5 * (0.25 / entry_vol)), clamped [3, 10]

Documented as "prometedor" in OMNICAPITAL_V7_IDEAS_RADICALES.md.
...
HOLD_DAYS = 5               # Base hold (used as reference)
HOLD_MIN = 3                # Shorten for high-vol stocks
HOLD_MAX = 5                # Don't extend beyond base (only shorten)
HOLD_VOL_BASELINE = 0.25    # Baseline annualized vol for scaling
...
print(f"Hold: DYNAMIC {HOLD_MIN}-{HOLD_MAX}d (base {HOLD_DAYS}d, vol baseline {HOLD_VOL_BASELINE:.0%}) | DD tiers: {DD_SCALE_TIER1}/{DD_SCALE_TIER2}/{DD_SCALE_TIER3}")
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    for i, date in enumerate(all_dates):
        ...
        for symbol in list(positions.keys()):
            ...
            pos_hold = pos.get('dynamic_hold', HOLD_DAYS)  # EXP57: per-position hold
            if days_held >= pos_hold:
            ...
            if len(current_scores) >= MIN_MOMENTUM_STOCKS and len(available_scores) >= needed:
                ...
                for symbol in selected:
                    ...
                    if cost + commission <= cash * 0.90:
                        ...
                        # EXP57: Dynamic hold based on entry vol
                        if entry_vol > 0.01:
                            dyn_hold = round(HOLD_DAYS * (HOLD_VOL_BASELINE / entry_vol))
                            dyn_hold = max(HOLD_MIN, min(HOLD_MAX, dyn_hold))
                        else:
                            dyn_hold = HOLD_DAYS
                        ...
                        positions[symbol] = {
                            ...
                            'dynamic_hold': dyn_hold,            # EXP57: vol-scaled hold
                        }
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp57_dynamic_hold_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp57_dynamic_hold_trades.csv', index=False)

    output_file = 'results_exp57_dynamic_hold.pkl'
    ...
    print(f"Daily CSV: backtests/exp57_dynamic_hold_daily.csv")
    print(f"Trades CSV: backtests/exp57_dynamic_hold_trades.csv")
