"""
OmniCapital v8.5a COMPASS - Stop Widening ONLY (breadth disabled)
==================================================================================
A/B test variant: ONLY regime-conditional stop widening, NO breadth regime change.
"""
...
def compute_regime_score(spy_data: pd.DataFrame, date: pd.Timestamp, price_data: Optional[Dict[str, pd.DataFrame]] = None, tradeable_symbols: Optional[List[str]] = None) -> float:
    ...
    # v8.5a: Breadth DISABLED — use original v84 weights
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/v85a_stop_only_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/v85a_stop_only_trades.csv', index=False)

    output_file = 'results_v85a_stop_only.pkl'
