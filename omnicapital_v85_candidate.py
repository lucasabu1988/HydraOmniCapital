"""
OmniCapital v8.5 COMPASS CANDIDATE — HAR-RV Adaptive Recovery
==================================================================================
Based on v8.4 COMPASS with ONE targeted improvement:

IDEA 1: HAR-RV Adaptive Recovery
  Replace fixed 10-day crash cooldown with volatility-gated re-entry.
  After crash brake fires, stay in CRASH_LEVERAGE until BOTH conditions met:
    A) HAR-RV 5d vol forecast < 1.35 × pre-crash realized vol (21d)
    B) At least 21 days since crash brake triggered
  Hard ceiling: 252 days (forced exit from crash mode).

  Source: Corsi (2009) "A Simple Approximate Long-Memory Model of Realized
  Volatility", Journal of Financial Econometrics.

  Pre-specified parameters (NOT optimized):
    HARV_VOL_THRESHOLD = 1.35  (from first principles: ~1 sigma above normal)
    HARV_MIN_DAYS = 21         (minimum floor to avoid whipsaw bounce)
    HARV_MAX_DAYS = 252        (hard ceiling, 1 trading year)

  Net new free parameters: 0

All v8.4 logic preserved:
- Signal: Cross-sectional momentum (90d) + short-term reversal (5d skip)
- Regime: Continuous sigmoid regime score [0,1] from trend (60%) + volatility (40%)
- Sizing: Inverse volatility weighting
- Leverage: Volatility targeting (auto-reduce en crisis)
- Smooth Drawdown Scaling: Piecewise-linear leverage reduction
- Exit Renewal: Winners can extend hold if profit > 4%, score in top 85%, hard cap 10d
- Quality Filter: Exclude stocks with vol > 60% or single-day moves > 50%
- Bull Market Override: +1 position when SPY > SMA200*1.03
- Adaptive Stops: Vol-scaled per position
- Sector Concentration: Max 3 per sector

Pre-registered success criteria (from decision doc 2026-03-01):
  - OOS CAGR improvement >= +60 bps vs baseline 13.90%
  - Max drawdown does not worsen by more than 100 bps
  - Sharpe ratio >= 1.20
  - Improvement must survive 2008 exclusion test

Pre-registered kill criteria:
  - OOS CAGR improvement < +20 bps
  - Max drawdown worsens by more than 150 bps
  - Improvement disappears when 2008-2009 excluded
"""
...
CRASH_COOLDOWN = 10         # Days of crash cooldown (v8.4 fallback, overridden by HAR-RV)

# v8.5: HAR-RV Adaptive Recovery (Idea 1)
# Pre-specified values — NOT optimized. Source: Corsi (2009).
HARV_VOL_THRESHOLD = 1.35   # Gate A: HAR forecast < threshold × pre-crash vol
HARV_MIN_DAYS = 21           # Gate B: minimum days before re-entry allowed
HARV_MAX_DAYS = 252          # Hard ceiling: forced exit from crash mode after 1 year
HARV_OLS_WINDOW = 252        # Rolling OLS window for HAR coefficient estimation
...
print("=" * 80)
print("OMNICAPITAL v8.5 COMPASS CANDIDATE — HAR-RV Adaptive Recovery")
print("v8.4 base + Idea 1: HAR-RV gated crash recovery")
print("=" * 80)
...
print(f"HAR-RV gate: vol_threshold={HARV_VOL_THRESHOLD}, min_days={HARV_MIN_DAYS}, max_days={HARV_MAX_DAYS}")
...
# ============================================================================
# v8.5: HAR-RV VOLATILITY FORECASTING (Corsi 2009)
# ============================================================================

def compute_realized_vol_components(spy_data: pd.DataFrame, date: pd.Timestamp) -> Optional[Tuple[float, float, float]]:
    """
    Compute HAR-RV components: daily, weekly (5d), monthly (21d) realized variance.
    Returns (RV_daily, RV_weekly, RV_monthly) as annualized volatilities,
    ...
    returns = spy_data['Close'].iloc[max(0, idx - 21):idx + 1].pct_change().dropna()
    if len(returns) < 20:
        return None

    # Squared returns as realized variance proxy
    sq_returns = returns ** 2

    # Daily RV: last day's squared return (annualized)
    rv_daily = float(sq_returns.iloc[-1]) * 252

    # Weekly RV: 5-day average of squared returns (annualized)
    rv_weekly = float(sq_returns.iloc[-5:].mean()) * 252

    # Monthly RV: 21-day average of squared returns (annualized)
    rv_monthly = float(sq_returns.mean()) * 252

    return (rv_daily, rv_weekly, rv_monthly)


def compute_har_forecast(spy_data: pd.DataFrame, date: pd.Timestamp) -> Optional[float]:
    """
    Compute HAR-RV 5-day ahead volatility forecast.

    HAR model (Corsi 2009):
        RV_forecast = alpha + beta_d * RV_daily + beta_w * RV_weekly + beta_m * RV_monthly

    Coefficients estimated via OLS on rolling 252-day window.
    Returns annualized volatility forecast (sqrt of forecast variance), or None.
    """
    if date not in spy_data.index:
        return None

    idx = spy_data.index.get_loc(date)
    # Need 252 + 21 days of history for OLS window + monthly RV
    if idx < HARV_OLS_WINDOW + 25:
        return None

    # Build training data: for each day in [idx-252, idx-1], compute RV components
    # and the 5-day forward realized variance as target
    X_rows = []
    y_rows = []

    for t in range(idx - HARV_OLS_WINDOW, idx - 5):
        if t < 22:
            continue
        returns_window = spy_data['Close'].iloc[max(0, t - 21):t + 1].pct_change().dropna()
        if len(returns_window) < 20:
            continue

        sq_ret = returns_window ** 2
        rv_d = float(sq_ret.iloc[-1]) * 252
        rv_w = float(sq_ret.iloc[-5:].mean()) * 252
        rv_m = float(sq_ret.mean()) * 252

        # 5-day forward RV (target)
        fwd_returns = spy_data['Close'].iloc[t + 1:t + 6].pct_change().dropna()
        if len(fwd_returns) < 4:
            continue
        rv_fwd = float((fwd_returns ** 2).mean()) * 252

        X_rows.append([1.0, rv_d, rv_w, rv_m])
        y_rows.append(rv_fwd)
    ...
    # Forecast using current RV components
    components = compute_realized_vol_components(spy_data, date)
    if components is None:
        return None

    rv_d, rv_w, rv_m = components
    forecast_var = beta[0] + beta[1] * rv_d + beta[2] * rv_w + beta[3] * rv_m

    # Annualized vol forecast (sqrt of variance, floored at 0)
    if forecast_var <= 0:
        return 0.01
    return float(np.sqrt(forecast_var))


def compute_pre_crash_vol(spy_data: pd.DataFrame, date: pd.Timestamp, lookback: int = 21) -> float:
    """
    Compute 21-day realized vol at the time of crash brake trigger.
    This is the 'pre-event baseline' used in Gate A.
    """
    if date not in spy_data.index:
    ...
    returns = spy_data['Close'].iloc[idx - lookback:idx + 1].pct_change().dropna()
    if len(returns) < lookback - 2:
        return 0.15
...
def compute_smooth_leverage(drawdown: float,
                             ...
                             current_idx: int,
                             crash_cooldown: int,
                             har_forecast: Optional[float] = None,
                             pre_crash_vol: Optional[float] = None,
                             crash_trigger_idx: Optional[int] = None) -> tuple:
    """
    v8.5: Compute leverage via smooth drawdown scaling + HAR-RV gated crash recovery.

    Crash brake trigger: same as v8.4 (5d=-6% or 10d=-10%).
    Crash brake EXIT: replaced with HAR-RV dual gate:
      Gate A: HAR-RV forecast < HARV_VOL_THRESHOLD × pre_crash_vol
      Gate B: days_since_crash >= HARV_MIN_DAYS
      Hard ceiling: days_since_crash >= HARV_MAX_DAYS → force exit

    Returns: (leverage, updated_cooldown, new_crash_trigger_idx, new_pre_crash_vol)
    """
    def _val(entry):
    ...
    new_crash_trigger_idx = crash_trigger_idx
    new_pre_crash_vol = pre_crash_vol

    if crash_cooldown > 0:
        # Already in crash mode — check HAR-RV exit conditions
        days_since_crash = current_idx - crash_trigger_idx if crash_trigger_idx is not None else 0

        # Hard ceiling: force exit after HARV_MAX_DAYS
        if days_since_crash >= HARV_MAX_DAYS:
            updated_cooldown = 0
            new_crash_trigger_idx = None
            new_pre_crash_vol = None
            return (_dd_leverage(drawdown), updated_cooldown, new_crash_trigger_idx, new_pre_crash_vol)

        # Check dual gate for exit
        gate_b = days_since_crash >= HARV_MIN_DAYS
        gate_a = False
        if har_forecast is not None and pre_crash_vol is not None and pre_crash_vol > 0:
            gate_a = har_forecast < HARV_VOL_THRESHOLD * pre_crash_vol

        if gate_a and gate_b:
            # Both gates passed — exit crash mode
            updated_cooldown = 0
            new_crash_trigger_idx = None
            new_pre_crash_vol = None
            return (_dd_leverage(drawdown), updated_cooldown, new_crash_trigger_idx, new_pre_crash_vol)
        ...
        # Check for new crash brake trigger
        ...
        if not in_crash and current_idx >= 10:
            ...
            if val_10d > 0:
                ...
                if ret_10d <= CRASH_VEL_10D:
                    in_crash = True
        if in_crash:
            # New crash event — record trigger index and pre-crash vol
            updated_cooldown = 1  # enter crash mode (>0 means in crash)
            new_crash_trigger_idx = current_idx
            # pre_crash_vol is passed in from main loop (computed from SPY)

    if in_crash:
        dd_lev = _dd_leverage(drawdown)
        crash_lev = min(CRASH_LEVERAGE, dd_lev)
        return (crash_lev, updated_cooldown, new_crash_trigger_idx, new_pre_crash_vol)
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    # v8.5: HAR-RV crash recovery state
    crash_trigger_idx = None    # trading day index when crash brake first fired
    pre_crash_vol = None        # 21d realized vol at crash trigger time
    har_crash_events = []       # log of crash events with HAR-RV details
    ...
    for i, date in enumerate(all_dates):
        ...
        # --- v8.5: HAR-RV forecast for crash recovery gate ---
        har_forecast = compute_har_forecast(spy_data, date)

        # If crash brake is about to fire, capture pre-crash vol
        if crash_cooldown == 0 and crash_trigger_idx is None:
            # Not in crash — compute pre-crash vol in case brake fires this iteration
            _pending_pre_crash_vol = compute_pre_crash_vol(spy_data, date)
        else:
            _pending_pre_crash_vol = pre_crash_vol

        # --- Smooth drawdown leverage (v8.5: HAR-RV gated) ---
        prev_cooldown = crash_cooldown
        dd_leverage_val, crash_cooldown, crash_trigger_idx, pre_crash_vol = compute_smooth_leverage(
            drawdown, portfolio_values, max(i - 1, 0), crash_cooldown,
            har_forecast=har_forecast,
            pre_crash_vol=pre_crash_vol,
            crash_trigger_idx=crash_trigger_idx
        )

        # If crash brake just triggered (transition from 0 to >0), record pre-crash vol
        if prev_cooldown == 0 and crash_cooldown > 0:
            pre_crash_vol = _pending_pre_crash_vol
            har_crash_events.append({
                'trigger_date': date,
                'trigger_idx': i,
                'pre_crash_vol': pre_crash_vol,
                'har_forecast_at_trigger': har_forecast,
                'drawdown_at_trigger': drawdown,
            })

        # If crash mode just exited, log the resolution
        if prev_cooldown > 0 and crash_cooldown == 0:
            if har_crash_events:
                last_event = har_crash_events[-1]
                last_event['exit_date'] = date
                last_event['exit_idx'] = i
                last_event['duration_days'] = i - last_event['trigger_idx']
                last_event['har_forecast_at_exit'] = har_forecast
                last_event['drawdown_at_exit'] = drawdown
        ...
        portfolio_values.append({
            ...
            'in_crash_mode': crash_cooldown > 0,
            'har_forecast': har_forecast,
        })
    ...
    return {
        ...
        'har_crash_events': har_crash_events,
    }
...
if __name__ == "__main__":
    ...
    # 5. Print results
    print("\n" + "=" * 80)
    print("RESULTS - OMNICAPITAL v8.5 COMPASS CANDIDATE")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"{'Metric':<25} {'v6':>12} {'v8.4 COMPASS':>12} {'Change':>12}")
    print("-" * 65)
    v6_cagr = 0.054
    v6_sharpe = 0.22
    v6_maxdd = -0.594
    ...
    results['portfolio_values'].to_csv('backtests/v85_idea1_har_recovery_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/v85_idea1_har_recovery_trades.csv', index=False)

    output_file = 'results_v85_har_recovery.pkl'
    with open(output_file, 'wb') as f:  # pickle: same format as v8.3
        pickle.dump({
            'params': {
                ...
                'har_vol_threshold': HARV_VOL_THRESHOLD,
                'har_min_days': HARV_MIN_DAYS,
                'har_max_days': HARV_MAX_DAYS,
                'version': 'v8.5_candidate_idea1_har_recovery',
            },
            'metrics': metrics,
            ...
            'har_crash_events': results.get('har_crash_events', []),
        }, f)

    print(f"\nResults saved: {output_file}")
    print(f"Daily CSV: backtests/v85_idea1_har_recovery_daily.csv")
    print(f"Trades CSV: backtests/v85_idea1_har_recovery_trades.csv")

    # ==========================================================================
    # v8.5 HAR-RV CRASH RECOVERY DIAGNOSTICS
    # ==========================================================================
    print("\n" + "=" * 80)
    print("v8.5 HAR-RV CRASH RECOVERY DIAGNOSTICS")
    print("=" * 80)
    ...
    # Crash mode analysis
    crash_days = int(df_pv['in_crash_mode'].sum()) if 'in_crash_mode' in df_pv.columns else 0
    print(f"\n--- HAR-RV Crash Recovery Stats ---")
    print(f"Total crash mode days:      {crash_days:>6} / {total_days} ({crash_days/total_days*100:.1f}%)")
    print(f"HAR-RV parameters:")
    print(f"  Vol threshold:            {HARV_VOL_THRESHOLD}")
    print(f"  Min days:                 {HARV_MIN_DAYS}")
    print(f"  Max days:                 {HARV_MAX_DAYS}")
    print(f"  OLS window:               {HARV_OLS_WINDOW}")

    # Individual crash events
    har_events = results.get('har_crash_events', [])
    print(f"\nCrash brake events:         {len(har_events)}")
    if har_events:
        print(f"\n{'Date':>12} {'Duration':>10} {'PreVol':>8} {'HARtrig':>8} {'HARexit':>8} {'DD@trig':>8} {'DD@exit':>8}")
        print("-" * 78)
        for ev in har_events:
            trig_date = ev['trigger_date'].strftime('%Y-%m-%d') if hasattr(ev['trigger_date'], 'strftime') else str(ev['trigger_date'])
            duration = ev.get('duration_days', '?')
            pre_vol = f"{ev.get('pre_crash_vol', 0):.1%}" if ev.get('pre_crash_vol') else 'N/A'
            har_trig = f"{ev.get('har_forecast_at_trigger', 0):.1%}" if ev.get('har_forecast_at_trigger') else 'N/A'
            har_exit = f"{ev.get('har_forecast_at_exit', 0):.1%}" if ev.get('har_forecast_at_exit') else 'N/A'
            dd_trig = f"{ev.get('drawdown_at_trigger', 0):.1%}"
            dd_exit = f"{ev.get('drawdown_at_exit', 0):.1%}" if ev.get('drawdown_at_exit') is not None else 'active'
            print(f"{trig_date:>12} {str(duration):>10} {pre_vol:>8} {har_trig:>8} {har_exit:>8} {dd_trig:>8} {dd_exit:>8}")

        durations = [ev['duration_days'] for ev in har_events if 'duration_days' in ev]
        if durations:
            print(f"\nAvg crash duration:         {np.mean(durations):.0f} days")
            print(f"Min crash duration:         {min(durations)} days")
            print(f"Max crash duration:         {max(durations)} days")

    # ==========================================================================
    # v8.5 vs v8.4 COMPARISON (PRE-REGISTERED CRITERIA)
    # ==========================================================================
    print("\n" + "=" * 80)
    print("v8.5 vs v8.4 — PRE-REGISTERED EVALUATION")
    print("=" * 80)

    baseline_v84 = {
        'CAGR': 0.1390,      # corrected baseline from decision doc
        'MaxDD': -0.2962,     # from v8.3 run (v8.4 uses same DD scaling)
        'Sharpe': 1.20,       # approximate baseline
    }
    ...
    cagr_delta = v85['CAGR'] - baseline_v84['CAGR']
    maxdd_delta = v85['MaxDD'] - baseline_v84['MaxDD']
    ...
    # Pre-registered success/kill evaluation
    print(f"\n--- Pre-Registered Criteria ---")

    # Success criteria
    cagr_success = cagr_delta >= 0.0060  # +60 bps
    maxdd_ok = maxdd_delta >= -0.0100    # not worse by >100 bps
    sharpe_ok = v85['Sharpe'] >= 1.20

    print(f"  CAGR delta >= +60 bps:    {cagr_delta*10000:+.0f} bps — {'PASS' if cagr_success else 'FAIL'}")
    print(f"  MaxDD not worse >100 bps: {maxdd_delta*10000:+.0f} bps — {'PASS' if maxdd_ok else 'FAIL'}")
    print(f"  Sharpe >= 1.20:           {v85['Sharpe']:.3f} — {'PASS' if sharpe_ok else 'FAIL'}")

    # Kill criteria
    cagr_kill = cagr_delta < 0.0020      # < +20 bps
    maxdd_kill = maxdd_delta < -0.0150   # worsens by >150 bps

    if cagr_kill:
        print(f"\n  KILL: CAGR improvement < +20 bps ({cagr_delta*10000:+.0f} bps) — NOISE TERRITORY")
    if maxdd_kill:
        print(f"\n  KILL: MaxDD worsened by > 150 bps ({maxdd_delta*10000:+.0f} bps)")

    # Overall verdict
    all_success = cagr_success and maxdd_ok and sharpe_ok
    any_kill = cagr_kill or maxdd_kill

    if any_kill:
        verdict = "KILLED — Does not meet minimum thresholds"
    elif all_success:
        verdict = "PASSED — All pre-registered criteria met. v8.5 candidate validated."
    else:
        verdict = "INCONCLUSIVE — Some criteria met, some not. Requires deeper analysis."

    print(f"\n{'='*65}")
    print(f"  VERDICT: {verdict}")
    print(f"{'='*65}")

    print(f"\nNote: Run 2008-exclusion test separately to check for single-episode artifact.")

    print("\n" + "=" * 80)
    print("COMPASS v8.5 CANDIDATE BACKTEST COMPLETE")
    print("=" * 80)
