"""
OmniCapital v8.4 COMPASS - Residual Momentum Experiment
==================================================================================
EXPERIMENT: Replace raw momentum scoring with residual (beta-adjusted) momentum.

Base: v8.4 COMPASS (bull override, adaptive stops, sector limits) — ALL UNCHANGED.
Change: compute_momentum_scores() subtracts beta * SPY_return from stock return,
        isolating idiosyncratic momentum that is more durable and less correlated.

Formula:
    stock_90d_return = close[T-5] / close[T-90] - 1
    spy_90d_return   = spy_close[T-5] / spy_close[T-90] - 1
    beta_i           = cov(stock_rets, spy_rets) / var(spy_rets)  over 90d
    residual_return  = stock_90d_return - beta_i * spy_90d_return
    skip_5d          = close[T] / close[T-5] - 1
    raw_score        = residual_return - skip_5d
    final_score      = raw_score / ann_vol
...
print("=" * 80)
print("OMNICAPITAL v8.4 COMPASS - RESIDUAL MOMENTUM EXPERIMENT")
print("Base v8.4 + Residual (beta-adjusted) Momentum Scoring")
print("=" * 80)
print(f"\nBroad pool: {len(BROAD_POOL)} stocks | Top-{TOP_N} annual rotation")
print(f"Signal: RESIDUAL Momentum {MOMENTUM_LOOKBACK}d (skip {MOMENTUM_SKIP}d) + Inverse Vol sizing")
print(f"Residual = stock_return - beta * SPY_return (idiosyncratic strength)")
...
# ============================================================================
# RESIDUAL MOMENTUM SCORING (THE ONLY CHANGE vs v8.4)
# ============================================================================
...
def compute_momentum_scores(price_data: Dict[str, pd.DataFrame],
    ...
    """
    Compute RESIDUAL risk-adjusted cross-sectional momentum score.
    Score = (stock_momentum - beta * spy_momentum) / realized_vol

    Strips out the systematic (market) component of returns, isolating
    idiosyncratic momentum that is more durable and less correlated.
    """
    scores = {}
    RISK_ADJ_VOL_WINDOW = 63

    # Prepare SPY data for this date
    spy_idx = None
    spy_momentum = None
    spy_returns_window = None
    ...
    if spy_data is not None:
        ...
        spy_clean = spy_data
        if hasattr(spy_clean.index, 'tz') and spy_clean.index.tz is not None:
            spy_clean = spy_data.copy()
            spy_clean.index = spy_clean.index.tz_localize(None)

        if date_n in spy_clean.index:
            spy_idx = spy_clean.index.get_loc(date_n)
            needed_spy = MOMENTUM_LOOKBACK + MOMENTUM_SKIP
            if spy_idx >= needed_spy:
                spy_close_skip = spy_clean['Close'].iloc[spy_idx - MOMENTUM_SKIP]
                spy_close_lookback = spy_clean['Close'].iloc[spy_idx - MOMENTUM_LOOKBACK]
                if spy_close_lookback > 0 and spy_close_skip > 0:
                    spy_momentum = (spy_close_skip / spy_close_lookback) - 1.0
                    # Pre-compute SPY daily returns over lookback window for beta calc
                    spy_returns_window = spy_clean['Close'].iloc[
                        spy_idx - MOMENTUM_LOOKBACK:spy_idx + 1
                    ].pct_change().dropna()
    ...
    for symbol in tradeable:
        ...
        # Raw momentum (same as v8.4)
        momentum_raw = (close_skip / close_lookback) - 1.0
        skip_5d = (close_today / close_skip) - 1.0

        # --- RESIDUAL MOMENTUM: subtract beta * SPY_return ---
        if spy_momentum is not None and spy_returns_window is not None:
            # Stock daily returns over same lookback window
            stock_returns_window = df['Close'].iloc[
                sym_idx - MOMENTUM_LOOKBACK:sym_idx + 1
            ].pct_change().dropna()

            # Align on dates
            aligned = pd.DataFrame({
                'stock': stock_returns_window,
                'spy': spy_returns_window
            }).dropna()

            if len(aligned) >= 30:
                beta = aligned['stock'].cov(aligned['spy']) / aligned['spy'].var()
            else:
                beta = 1.0  # fallback: assume market beta

            residual_momentum = momentum_raw - beta * spy_momentum
        else:
            # Fallback: no SPY data available, use raw momentum
            residual_momentum = momentum_raw

        raw_score = residual_momentum - skip_5d

        # Risk adjustment: divide by realized vol (same as v8.4)
...
# ============================================================================
# REMAINING FUNCTIONS (identical to v8.4)
# ============================================================================
...
# ============================================================================
# SMOOTH DRAWDOWN SCALING (identical to v8.4)
# ============================================================================
...
# ============================================================================
# BACKTEST (identical to v8.4 except compute_momentum_scores call)
# ============================================================================
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    print("\n" + "=" * 80)
    print("RUNNING RESIDUAL MOMENTUM BACKTEST")
    print("=" * 80)
    ...
    for i, date in enumerate(all_dates):
        ...
        # --- Open new positions (identical to v8.4) ---
...
if __name__ == "__main__":
    ...
    # 3. Run residual momentum backtest
    ...
    # 5. Print results
    print("\n" + "=" * 80)
    print("RESULTS - RESIDUAL MOMENTUM EXPERIMENT")
    print("=" * 80)
    ...
    results['portfolio_values'].to_csv('backtests/v84_residual_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/v84_residual_trades.csv', index=False)

    output_file = 'results_v84_residual.pkl'
    with open(output_file, 'wb') as f:
        pickle.dump({
            'params': {
                ...
                'scoring': 'RESIDUAL momentum (beta-adjusted)',
    ...
    print(f"Daily CSV: backtests/v84_residual_daily.csv")
    print(f"Trades CSV: backtests/v84_residual_trades.csv")

    # ==========================================================================
    # A/B COMPARISON: RESIDUAL vs BASE v8.4
    # ==========================================================================
    print("\n" + "=" * 80)
    print("A/B COMPARISON: RESIDUAL MOMENTUM vs BASE v8.4")
    print("=" * 80)

    # Load base v8.4 results if available
    base_metrics = None
    base_pkl = 'results_v84_compass.pkl'
    base_csv = 'backtests/v84_compass_daily.csv'

    if os.path.exists(base_pkl):
        try:
            with open(base_pkl, 'rb') as f:
                base_data = pickle.load(f)
            base_metrics = base_data.get('metrics', None)
            print("[Loaded base v8.4 metrics from pickle]")
        except Exception:
            pass

    if base_metrics is None and os.path.exists(base_csv):
        try:
            base_df = pd.read_csv(base_csv, parse_dates=['date']).set_index('date')
            base_initial = INITIAL_CAPITAL
            base_final = base_df['value'].iloc[-1]
            base_years = len(base_df) / 252
            base_cagr = (base_final / base_initial) ** (1 / base_years) - 1
            base_returns = base_df['value'].pct_change().dropna()
            base_vol = base_returns.std() * np.sqrt(252)
            base_sharpe = base_cagr / base_vol if base_vol > 0 else 0
            base_maxdd = base_df['drawdown'].min()
            base_downside = base_returns[base_returns < 0]
            base_downside_vol = base_downside.std() * np.sqrt(252) if len(base_downside) > 0 else base_vol
            base_sortino = base_cagr / base_downside_vol if base_downside_vol > 0 else 0
            base_metrics = {
                'cagr': base_cagr,
                'sharpe': base_sharpe,
                'max_drawdown': base_maxdd,
                'sortino': base_sortino,
                'win_rate': 0,  # Not available from daily CSV
                'volatility': base_vol,
            }
            print("[Computed base v8.4 metrics from daily CSV]")
        except Exception as e:
            print(f"[Could not load base v8.4 results: {e}]")

    if base_metrics:
        print(f"\n{'':15s} | {'CAGR':>8s} | {'Sharpe':>8s} | {'MaxDD':>8s} | {'Sortino':>8s} | {'Vol':>8s}")
        print("-" * 72)
        print(f"{'Base v8.4':15s} | {base_metrics['cagr']:>7.2%} | {base_metrics['sharpe']:>8.2f} | {base_metrics['max_drawdown']:>7.2%} | {base_metrics['sortino']:>8.2f} | {base_metrics['volatility']:>7.2%}")
        print(f"{'Residual':15s} | {metrics['cagr']:>7.2%} | {metrics['sharpe']:>8.2f} | {metrics['max_drawdown']:>7.2%} | {metrics['sortino']:>8.2f} | {metrics['volatility']:>7.2%}")
        delta_cagr = metrics['cagr'] - base_metrics['cagr']
        delta_sharpe = metrics['sharpe'] - base_metrics['sharpe']
        delta_maxdd = metrics['max_drawdown'] - base_metrics['max_drawdown']
        delta_sortino = metrics['sortino'] - base_metrics['sortino']
        delta_vol = metrics['volatility'] - base_metrics['volatility']
        print(f"{'Delta':15s} | {delta_cagr:>+7.2%} | {delta_sharpe:>+8.2f} | {delta_maxdd:>+7.2%} | {delta_sortino:>+8.2f} | {delta_vol:>+7.2%}")

        # Kill criteria check
        print(f"\n--- Kill Criteria ---")
        cagr_ok = delta_cagr > -0.005  # CAGR delta > -0.5%
        maxdd_ok = delta_maxdd > 0.01   # MaxDD improvement > +1%
        print(f"  CAGR delta > -0.5%:    {delta_cagr:>+.2%} -> {'PASS' if cagr_ok else 'FAIL'}")
        print(f"  MaxDD improvement > 1%: {delta_maxdd:>+.2%} -> {'PASS' if maxdd_ok else 'FAIL'}")

        if cagr_ok and maxdd_ok:
            print(f"\n  VERDICT: PASS -- Residual momentum is viable for further testing")
        elif cagr_ok:
            print(f"\n  VERDICT: MARGINAL -- CAGR acceptable but MaxDD not improved enough")
        else:
            print(f"\n  VERDICT: FAIL -- Residual momentum does not pass kill criteria")
    else:
        print("\n[No base v8.4 results found. Run omnicapital_v84_compass.py first for A/B comparison.]")
        print(f"\nStandalone residual momentum results:")
        print(f"  CAGR:     {metrics['cagr']:>8.2%}")
        print(f"  Sharpe:   {metrics['sharpe']:>8.2f}")
        print(f"  MaxDD:    {metrics['max_drawdown']:>8.2%}")
        print(f"  Sortino:  {metrics['sortino']:>8.2f}")

    print("\n" + "=" * 80)
    print("RESIDUAL MOMENTUM EXPERIMENT COMPLETE")
    print("=" * 80)
