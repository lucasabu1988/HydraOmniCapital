"""
EXP56: Multi-Lookback Momentum Ensemble
==================================================================================
Based on v8.4 COMPASS with ONE change: replace single-lookback momentum
with an ENSEMBLE of 4 lookback windows.

CHANGE: Instead of ranking by 90d momentum alone, we:
  1. Compute risk-adjusted momentum at 21d, 63d, 126d, 252d
  2. Convert each to percentile rank within the universe
  3. Blend with equal weights: score = mean(rank_21, rank_63, rank_126, rank_252)

This captures momentum at multiple frequencies — short-term (1mo),
medium-term (3mo), intermediate (6mo), and long-term (12mo).
Reduces dependence on any single lookback window.
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp56_multi_lookback_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp56_multi_lookback_trades.csv', index=False)

    output_file = 'results_exp56_multi_lookback.pkl'
    ...
    print(f"Daily CSV: backtests/exp56_multi_lookback_daily.csv")
    print(f"Trades CSV: backtests/exp56_multi_lookback_trades.csv")
