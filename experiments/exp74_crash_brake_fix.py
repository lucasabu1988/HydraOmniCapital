...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    for i, date in enumerate(all_dates):
        ...
        # FIX (issue #36): when crash brake is active, dd_leverage_val drops to
        # CRASH_LEVERAGE (0.15), which is BELOW LEV_FLOOR (0.30). The original
        # `max(..., LEV_FLOOR)` clamped it back up to 0.30, neutralizing the
        # emergency brake. Mirror omnicapital_live.py:1444-1446 by bypassing
        # the floor whenever the crash brake fired.
        _combined = min(dd_leverage_val, vol_leverage)
        if dd_leverage_val < LEV_FLOOR:
            current_leverage = _combined  # crash brake bypass
        else:
            current_leverage = max(_combined, LEV_FLOOR)
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp74_crash_brake_fix_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp74_crash_brake_fix_trades.csv', index=False)

    output_file = 'results_exp74_crash_brake_fix.pkl'
