"""
OmniCapital v8.4 COMPASS - 3 Algorithmic Improvements
==================================================================================
...
3. SECTOR CONCENTRATION: Max 2 positions per GICS sector (prevents
...
- Smooth Drawdown Scaling: Piecewise-linear leverage reduction + crash velocity circuit breaker
...
def run_backtest(price_data: Dict[str, pd.DataFrame],
    ...
    print("\n" + "=" * 80)
    print("RUNNING COMPASS BACKTEST (exp75 — NEXT-OPEN EXECUTION MODE)")
    print("=" * 80)
    print("Signal: Close[T].  Execution: Open[T+1].  Stops: gap-through modeled.")
    ...
    for i, date in enumerate(all_dates):
        ...
        for symbol in list(positions.keys()):
            ...
            if exit_reason:
                # exp75: NEXT-OPEN execution. Signal fires at Close[T], order is
                # submitted to fill at Open[T+1]. For stop-loss exits we also
                # model gap-through: if Open[T+1] is below the stop level, the
                # realistic fill is the open price (worse than the stop level).
                if i + 1 >= len(all_dates):
                    continue  # last bar of the backtest — can't execute next day
                next_date = all_dates[i + 1]
                if next_date not in price_data[symbol].index:
                    continue  # symbol doesn't trade next day — carry position
                exit_price = float(price_data[symbol].loc[next_date, 'Open'])
                if exit_price <= 0:
                    continue
                # Gap-through handling: stop orders that gap past the stop fill at Open
                # (already captured by using Open[T+1]). Non-gap stops still execute at Open[T+1]
                # because the signal was generated at Close[T] and the order is submitted
                # MOO (market-on-open) for T+1.
        ...
        if needed > 0 and cash > 1000 and len(tradeable_symbols) >= 5:
            ...
            if len(current_scores) >= MIN_MOMENTUM_STOCKS and len(available_scores) >= needed:
                ...
                for symbol in selected:
                    ...
                    # exp75: NEXT-OPEN execution. Signal fires at Close[T], MOO order
                    # fills at Open[T+1]. This is the most conservative realistic
                    # execution model — overnight gap risk fully borne by the strategy.
                    if i + 1 >= len(all_dates):
                        continue  # last bar of the backtest
                    next_date = all_dates[i + 1]
                    if next_date not in price_data[symbol].index:
                        continue  # symbol doesn't trade next day
                    entry_price = float(price_data[symbol].loc[next_date, 'Open'])
                    if entry_price <= 0:
                        continue
                    ...
                    if cost + commission <= cash * 0.90:
                        # v8.4: Compute entry-time vol for adaptive stops (using signal-day
                        # vol — same data that was used to rank the candidate)
                        entry_vol, entry_daily_vol = compute_entry_vol(price_data, symbol, date)

                        # exp75: position is recorded as entering at T+1 (the fill bar), so
                        # hold_days counts forward correctly from the fill, not from the signal.
                        positions[symbol] = {
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp75_next_open_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp75_next_open_trades.csv', index=False)

    output_file = 'results_exp75_next_open.pkl'
    ...
    print(f"Daily CSV: backtests/v84_compass_daily.csv")
    print(f"Trades CSV: backtests/v84_compass_trades.csv")
