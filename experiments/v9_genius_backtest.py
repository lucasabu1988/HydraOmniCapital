...
MLP_RETRAIN_FREQ = 21           # Retrain monthly (21 trading days)
...
class MLPFilter:
    ...
    def train(self, price_data, tradeable_symbols, date, date_idx, all_dates):
        ...
        # Sample every 5 days to reduce correlation and speed up
        sample_indices = range(train_start, date_idx - HOLD_DAYS - 1, 5)
    ...
    for i, date in enumerate(all_dates):
        ...
        if needed > 0 and cash > 1000 and len(tradeable_symbols) >= 5:
            ...
            if len(available_scores) >= needed:
                ...
                # Get top candidates (70th percentile by MLP score)
                if len(mlp_scores) > 5:
