...
**Objetivo declarado:** retorno por unidad de riesgo, no retorno absoluto. Evidencia (simulación con contabilidad ejecutable, panel S&P 500 point-in-time con delistings, costes incluidos): el **motor de producción** end-to-end (plan/settle/mark, 1083 planes semanales 2005-2026, TASK-350) da **7.10 % neto, ratio retorno/volatilidad 0.75, maxDD -17.8 %**, y su **Sharpe real** — restando el T-bill, que corrió al 1.76 % anualizado en la muestra — es **0.57**. La mezcla del laboratorio que lo aproxima da 6.91 / 0.74 / -19.5 (Sharpe 0.56), contra el screener v8.4 solo (5.5 % / 0.42 / -37.8 %, Sharpe 0.31) y SPY comprar-y-mantener (11.0 % / 0.69 / -52.5 %, Sharpe 0.59); estas dos son las **líneas de comparación**, no el motor, vienen de la auditoría 2026-09-06 y están **recomputadas el 2026-09-08** con la métrica corregida sobre la misma rejilla de marcas (1084 pasos de 5 barras, [`experiments/reference_rows.py`](hydra_screener_local/experiments/reference_rows.py)) — el maxDD de SPY baja a -52.5 % porque la auditoría lo había medido en otra rejilla. El 10 % neto **no se demuestra**; el universo Russell de producción **no está medido** (solo S&P 500). No hay track record: el único registro vivo es el ledger de `state/`.

Estas cifras se generan desde [`evidence_canonical.json`](hydra_screener_local/evidence_canonical.json) con `render_evidence.py` (TASK-408); medidas el 2026-09-08 en `7acf22a`.
<!-- EVIDENCE:END -->
