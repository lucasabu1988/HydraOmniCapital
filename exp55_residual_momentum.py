"""
EXP55: Residual Momentum (Market-Neutral)
==================================================================================
Based on v8.4 COMPASS with ONE change: replace raw momentum scoring with
RESIDUAL MOMENTUM (market-neutral).

CHANGE: Blend raw momentum (70%) with residual momentum (30%) using
rank-based combination. This partially strips market beta while
preserving the raw signal that works in large-cap S&P 500 universe.
...
BLEND_RAW_WEIGHT = 0.70     # Weight for raw momentum rank
BLEND_RESID_WEIGHT = 0.30   # Weight for residual momentum rank
...
print(f"Signal: BLENDED Momentum {MOMENTUM_LOOKBACK}d ({BLEND_RAW_WEIGHT:.0%} raw + {BLEND_RESID_WEIGHT:.0%} residual, reg {REGRESSION_WINDOW}d) + Inverse Vol sizing")
...
def compute_momentum_scores(price_data: Dict[str, pd.DataFrame],
    ...
    """
    EXP55b: Blended momentum = 70% raw rank + 30% residual rank.
    Uses rank-based blending to avoid scale mismatch between raw and residual scores.
    """
    ...
    for symbol in tradeable:
        ...
        if vol_window >= 20:
            ...
            if len(returns) >= 15:
                ...
                if ann_vol > 0.01:
                    raw_scores[symbol] = raw_score / ann_vol
                else:
    ...
    # --- Step 2: Compute RESIDUAL momentum scores ---
    resid_scores = {}
    if spy_data is not None and date in spy_data.index:
        spy_idx = spy_data.index.get_loc(date)
        needed_spy = REGRESSION_WINDOW + MOMENTUM_SKIP
        if spy_idx >= needed_spy:
            spy_returns_full = spy_data['Close'].iloc[spy_idx - REGRESSION_WINDOW:spy_idx + 1].pct_change().dropna()
            ...
            for symbol in tradeable:
                ...
                try:
                    ...
                    continue
                needed = REGRESSION_WINDOW + MOMENTUM_SKIP
                if sym_idx < needed:
                    continue

                stock_returns_full = df['Close'].iloc[sym_idx - REGRESSION_WINDOW:sym_idx + 1].pct_change().dropna()
                common_idx = spy_returns_full.index.intersection(stock_returns_full.index)
                if len(common_idx) < 120:
                    continue

                stock_ret = stock_returns_full.loc[common_idx].values
                spy_ret = spy_returns_full.loc[common_idx].values

                spy_var = np.var(spy_ret, ddof=1)
                if spy_var < 1e-12:
                    continue
                beta = np.cov(stock_ret, spy_ret, ddof=1)[0, 1] / spy_var
                alpha = stock_ret.mean() - beta * spy_ret.mean()
                residuals = stock_ret - alpha - beta * spy_ret

                n_common = len(common_idx)
                mom_end = n_common - MOMENTUM_SKIP
                mom_start = max(0, mom_end - MOMENTUM_LOOKBACK)
                if mom_end <= mom_start or mom_end < 1:
                    continue

                resid_window = residuals[mom_start:mom_end]
                if len(resid_window) < 30:
                    continue

                resid_momentum = float(np.sum(resid_window))
                resid_vol = float(np.std(residuals, ddof=1)) * (252 ** 0.5)
                if resid_vol > 0.01:
                    resid_scores[symbol] = resid_momentum / resid_vol
                else:
                    resid_scores[symbol] = resid_momentum

    # --- Step 3: Rank-based blending ---
    # Only blend symbols that have BOTH scores
    common_symbols = set(raw_scores.keys()) & set(resid_scores.keys())
    if not common_symbols:
        return raw_scores

    # Convert to percentile ranks within common set
    raw_sorted = sorted(common_symbols, key=lambda s: raw_scores[s])
    resid_sorted = sorted(common_symbols, key=lambda s: resid_scores[s])
    n = len(common_symbols)

    raw_rank = {s: i / (n - 1) if n > 1 else 0.5 for i, s in enumerate(raw_sorted)}
    resid_rank = {s: i / (n - 1) if n > 1 else 0.5 for i, s in enumerate(resid_sorted)}

    blended = {}
    for s in common_symbols:
        blended[s] = BLEND_RAW_WEIGHT * raw_rank[s] + BLEND_RESID_WEIGHT * resid_rank[s]

    # Add symbols that only have raw scores (no residual available)
    for s in raw_scores:
        if s not in blended:
            blended[s] = raw_scores[s]
...
if __name__ == "__main__":
    ...
    results['portfolio_values'].to_csv('backtests/exp55_residual_momentum_daily.csv', index=False)
    if len(results['trades']) > 0:
        results['trades'].to_csv('backtests/exp55_residual_momentum_trades.csv', index=False)

    output_file = 'results_exp55_residual_momentum.pkl'
    ...
    print(f"Daily CSV: backtests/exp55_residual_momentum_daily.csv")
    print(f"Trades CSV: backtests/exp55_residual_momentum_trades.csv")
